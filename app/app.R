    ...
    fluidRow(
      ...
      tabsetPanel(
        ...
        tabPanel("Visualizations",
                 br(), h5('Choose an artist and variables above. Then click Submit 
...
server <- function(input, output, session) {
  ...
  output$welcome <- renderText({
    paste("Welcome to the analysis page for ", input$artist, "!", sep = "")
  })
  ...
  output$right_graph <- renderPlot({
    ...
    req(reactives$rstat)
    makeGraph(reactives$rstat, reactives$artist, 'forestgreen')
  })
